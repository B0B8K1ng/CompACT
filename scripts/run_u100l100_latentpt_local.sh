...
# u100l100 only: extract step-40000 actions, then repeat the completed
# u25/u50/u75l100 training contract. Derived from wait_and_run_nwm_latentpt_local.sh.
...
SUPERVISOR_LOG="${SUPERVISOR_LOG:-${OUTPUT_PARENT}/u100l100_supervisor.log}"
STATUS_FILE="${OUTPUT_PARENT}/u100l100_supervisor.status"
...
CHECKPOINT=/data1/ndj/checkpoints/hongyu/LAM-U100L100.ckpt
[[ -f "${CHECKPOINT}" ]] || fail "missing step=40000 checkpoint: ${CHECKPOINT}; ModelScope download required"
EXTRACTION_ROOTS=(/data1/ndj/LAM-Data/navv1_latent_actions_u100l100_step40000)
EXPERIMENT_TAGS=(u100l100)
CHECKPOINT_SHA256S=("$(sha256sum "${CHECKPOINT}" | cut -d ' ' -f 1)")
[[ "${CHECKPOINT_SHA256S[0]}" == a6deb2f3f00c8ed23dcd7bf415ef49ef020fc11eb8d264d806bed1d5b7d92347 ]] || \
    fail "u100l100 checkpoint SHA256 differs from ModelScope step40000 manifest"
export REUSE_VALIDATED_BATCH=48
unset RESUME_U25_RUN_ID
...
set_status VERIFYING_CHECKPOINT "u100l100 step=40000"
/home/user/ndj/code/DreamDojo/.venv/bin/python - "${CHECKPOINT}" <<'VERIFY'
import sys
sys.path.insert(0, '/home/user/ndj/code/CompACT')
from pathlib import Path
from precompute_navanywhere_nav1_latent_actions import _lam_api
inspect, _, _ = _lam_api(Path('/home/user/ndj/code/DreamDojo/external/lam_project'))
metadata = inspect(Path(sys.argv[1]))
if int(metadata['global_step']) != 40000:
    raise SystemExit(f"Expected step 40000, got {metadata['global_step']}")
print('Verified LAM global_step=40000', flush=True)
VERIFY
if [[ ! -f "${EXTRACTION_ROOTS[0]}/_SUCCESS.json" ]]; then
    set_status EXTRACTING "u100l100; 8 GPUs, 3 workers/GPU, batch 24, compiled encoder"
    extraction_ok=0
    for extraction_attempt in 1 2 3; do
      if OMP_NUM_THREADS=1 HF_HUB_OFFLINE=1 PYTHONUNBUFFERED=1 \
      /home/user/ndj/code/DreamDojo/.venv/bin/python -u \
      "${REPO_ROOT}/scripts/precompute_finetune_nav1_actions.py" run \
      --output-root "${EXTRACTION_ROOTS[0]}" --checkpoint "${CHECKPOINT}" \
      --data-root /data1/ndj/LAM-Data --split-root "${REPO_ROOT}/data_splits" \
      --lam-root /home/user/ndj/code/DreamDojo/external/lam_project \
      --gpus 0,1,2,3,4,5,6,7 --workers-per-gpu 3 --batch-size 24 \
      --compile-encoder --loader-threads 4; then
        extraction_ok=1
        break
      fi
      echo "$(date -u +%FT%TZ) extraction attempt ${extraction_attempt}/3 failed; completed trajectories will be reused"
      (( extraction_attempt == 3 )) || sleep 20
    done
    (( extraction_ok == 1 )) || fail "u100l100 extraction failed after three attempts"
fi

set_status WAITING_EXTRACTIONS "u100l100"
...
for index in "${!EXTRACTION_ROOTS[@]}"; do
    ...
    set_status TRAINING "experiment=$((index + 1))/${#EXTRACTION_ROOTS[@]} tag=${tag} run_id=${run_id} batch=${chosen_batch} log=${run_log}"
