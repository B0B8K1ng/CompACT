"""Evaluate verified U100L100 checkpoint on five full splits."""
...
OUT=Path('/file_system/nas/algorithm/dujun.nie/nwm/results/nav1_u100l100_direct4s_20260926')
...
def prepare(label):
 ...
 remote=json.loads((WEIGHTS/'remote_manifest_u100l100.json').read_text())
...
def gpu_queue(gpu,datasets):
 return [worker(d,gpu) for d in datasets]
with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
 futures=[pool.submit(gpu_queue,3,['recon','tartan_drive','huron']),pool.submit(gpu_queue,5,['scand','go_stanford'])]
...
comparison=json.loads((OLD/'results.json').read_text())+json.loads(Path('/file_system/nas/algorithm/dujun.nie/nwm/results/nav1_u25l50_direct4s_20260925/results.json').read_text())+json.loads(Path('/file_system/nas/algorithm/dujun.nie/nwm/results/nav1_u25l0_direct4s_20260925/results.json').read_text())+rows
