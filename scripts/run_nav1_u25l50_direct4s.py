...
OUT=Path('/file_system/nas/algorithm/dujun.nie/nwm/results/nav1_u25l50_direct4s_20260925')
...
def prepare(label):
 ...
 remote=json.loads((WEIGHTS/'remote_manifest_u25l50.json').read_text())
...
with concurrent.futures.ThreadPoolExecutor(max_workers=5) as pool:
 rows=list(pool.map(worker,['recon'] if '--recon-only' in sys.argv else DATASETS,[0] if '--recon-only' in sys.argv else [2,5,6,7,0]))
...
comparison=json.loads((OLD/'results.json').read_text())+rows
