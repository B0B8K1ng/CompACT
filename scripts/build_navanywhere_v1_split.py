#!/usr/bin/env python3
"""Freeze a trajectory-disjoint split of the original 13-source NavAnywhere v1."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--inventory-recipe",
        required=True,
        help="The frozen original 13-source v1 recipe used as the inventory.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="Logical validation slots per source; evaluation still consumes one batch.",
    )
    parser.add_argument(
        "--expected-source",
        action="append",
        default=None,
        help="Expected source ID; defaults to the canonical v1 13-source set.",
    )
...
def _tie_break(seed: int, source: str, trajectory: str) -> str:
    return hashlib.sha256(
        f"navanywhere-v1-val\0{seed}\0{source}\0{trajectory}".encode("utf-8")
    ).hexdigest()
...
def choose_validation_trajectories(
    ...
    *,
    ...
    """Select a useful holdout while removing little data from v1 training."""
...
def main() -> None:
    ...
    if len(expected_sources) != 13:
        raise ValueError(
            f"NavAnywhere v1 requires exactly 13 sources, got {len(expected_sources)}"
        )

    recipe, inventory_path, inventory_digest = load_sampling_recipe(
        args.inventory_recipe,
        seed=args.seed,
        context_size=args.context_size,
        goals_per_obs=args.goals_per_obs,
        frame_offset_range=(-64, 64),
    )
    inventory = [dict(item) for item in recipe["trajectories"]]
    actual_sources = {str(item["source_id"]) for item in inventory}
    if actual_sources != expected_sources:
        raise ValueError(
            "13-source v1 inventory mismatch: "
    ...
    training = [
        item
        for item in inventory
        if (str(item["source_id"]), str(item["trajectory_id"]))
    ...
    if (
        set(train_recipe["sources"]) != expected_sources
        or set(val_recipe["sources"]) != expected_sources
    ):
        raise RuntimeError("Both v1 split recipes must retain all 13 sources")
    ...
    for source in sorted(expected_sources):
        ...
        per_source[source] = {
            ...
            "train_observations": sum(
                int(item["observation_count"]) for item in source_train
            ),
            "validation": [
                {
    ...
    report = {
        ...
        "format": SPLIT_FORMAT,
        "dataset_version": "v1_13_sources",
        "seed": int(args.seed),
        "policy": {
            "unit": "trajectory",
            "validation_trajectories_per_source": int(
                args.val_trajectories_per_source
            ),
        ...
        "totals": {
            ...
            "all_observations": sum(
                int(item["observation_count"]) for item in inventory
            ),
