...
| 文件 | 表格数 | 内容 |
| --- | --- | --- |
| `navigation_main_comparison.tex` | 2 | 导航规划 ATE/RPE；视觉预测 LPIPS/DreamSim/FID，按 RECON 自回归预测与 SACSoN 单步预测分组 |
...
- 主结果分为两张独立表，仍放在同一个 tex 文件中：规划表使用 `table`，标签为 `tab:main_sota_navigation`；视觉表使用 `table*`，标签为 `tab:main_visual_prediction`。两表各有标题和编号。视觉表按 RECON 自回归预测（4 FPS）与 SACSoN 直接单步预测分组；4 秒和 16 秒横向并列，每个时域下依次为 LPIPS、DreamSim、FID，每个方法在组内只占一行。每组保留同设置 NWM 基线，只能在同组、同时域内比较。
- 预训练预测表把重复的 Hours 列移到表注：NA-v1 为 322 小时，NA-v2 为 1200+ 小时。模型名和数据版本保持原命名。
- 主表的外部结果已核对原文。规划表只纳入明确沿用 NWM 的 RECON 2 秒规划协议的结果；视觉表采用各论文内同设置对比，不声称跨论文的测试索引或协议相同。其他主题文件的外部引用值未在本次重新核验。
- 规划表要求 CEM 120 个候选、1 次迭代。本项目消融中的 CEM-80 结果不能直接搬入该表。视觉表的本方法结果也须按相应数据集、预测方式、时域与评测设置重新核对后填入。
- 两张主表各自只展示对应任务的指标；拆分保留原有数值、精度与待评测结果。
...
`preview.tex` 按主题载入全部 22 张表，每个主题结束后换页；主题内部由 LaTeX 自动排版。论文正文可直接引用所需主题文件。
