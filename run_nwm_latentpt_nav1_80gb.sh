...
case "${GPU_MEMORY_GB}" in
    ...
    48) DEFAULT_BATCH_SIZE=32; DEFAULT_TOTAL_GPU_MB=45000; DEFAULT_FREE_GPU_MB=44000 ;;
...
STAGE1_SAMPLES_PER_RANK=3200000
WARMUP_SAMPLES_PER_RANK=160000
JOINT_SAMPLES_PER_RANK=1600000
REFERENCE_SAMPLES_PER_RANK_PER_EPOCH=516544
...
if [[ "${BATCH_SIZE}" != "96" && "${BATCH_SIZE}" != "64" && "${BATCH_SIZE}" != "32" && "${BATCH_SIZE}" != "16" ]]; then
    echo "ERROR: BATCH_SIZE must be 16, 32, 64, or 96." >&2
    exit 2
fi
...
if [[ "${RESULTS_ROOT}" != /file_system/nas/algorithm/dujun.nie/* ]]; then
    echo "ERROR: RESULTS_ROOT must stay on /file_system/nas/algorithm/dujun.nie." >&2
    exit 2
fi
...
STAGE1_OVERRIDES=(
    ...
    "training.notes=Nav1_LatentPT_${HARDWARE_LABEL}_bs${BATCH_SIZE}"
    "training.wandb_tags=[Nav1,LatentPT,CDiT-B,${HARDWARE_LABEL},bs${BATCH_SIZE}]"
...
run_stage() {
    ...
    if (( status != 0 )); then
        ...
        echo "If this was CUDA OOM, start a new RUN_ID with a smaller BATCH_SIZE (96 -> 64 -> 32 -> 16)." >&2
        return "${status}"
    fi
