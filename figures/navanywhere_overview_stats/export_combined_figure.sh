...
output_dir=/file_system/vepfs/algorithm/dujun.nie/.codex/visualizations/2026/09/22/01a0c742-f9c0-7ab1-9bd0-dca2be5385fa/navanywhere_overview_stats_combined_20260922
python_bin=/file_system/vepfs/algorithm/dujun.nie/miniconda3/bin/python
chrome_bin=/opt/google/chrome/google-chrome
base=navanywhere_overview_with_statistics_20260922

mkdir -p "$output_dir"

"$python_bin" "$script_dir/build_combined_figure.py" --output-dir "$output_dir"
cp "$script_dir/build_combined_figure.py" "$output_dir/build_combined_figure.py"
cp "$script_dir/export_combined_figure.sh" "$output_dir/export_combined_figure.sh"
...
(
  ...
  sha256sum \
    ...
    scene_composition.csv \
    confirmed_dataset_composition_reference.png \
    confirmed_scene_composition_reference.png \
    build_combined_figure.py \
    export_combined_figure.sh \
    > SHA256SUMS.txt
)
...
zip -j -9 -q \
  ...
  "$output_dir/build_combined_figure.py" \
  "$output_dir/export_combined_figure.sh"
