...
def log_viz_single(
    ...
    """
    viz_obs_image = obs_image.detach().cpu()[-1]  # take last img
    viz_goal_image = goal_image.detach().cpu()
...
def plot_images_and_actions(
    ...
    curr_viz_pred_actions = curr_viz_pred_actions * waypoint_spacing
    curr_viz_actions = curr_viz_actions * waypoint_spacing
    ...
    # Sort losses and get indices of top-3 lowest loss values
    _, sorted_indices = torch.sort(loss)
    top_indices = sorted_indices[:3]  # Get indices of 3 lowest loss values

    # Plot only the top-3 lowest loss trajectories
    for idx_rank, i in enumerate(top_indices):
        if i == min_idx.item():
            continue  # Skip the minimum loss sample here, will plot it separately

        color = colors[idx_rank % len(colors)]
        label = f"Sample {i} (Rank {idx_rank + 1})"
    ...
    axs[2].plot(
        ...
        marker="o",
        markersize=5,
        label=f"Sample {min_idx.item()} (Best)",
    )
    axs[2].text(
    ...
    plot_array = np.asanyarray(buf, dtype="uint8")[..., :3]
...
def get_unnormalize(mean: list = [0.5, 0.5, 0.5], std: list = [0.5, 0.5, 0.5]):
...
def get_normalize(mean: list = [0.5, 0.5, 0.5], std: list = [0.5, 0.5, 0.5]):
    return transforms.Normalize(
        mean=mean,
        std=std,
    )
